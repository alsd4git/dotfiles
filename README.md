...
The current Windows prompt theme is tracked in `windows/omp/tokyo.omp.json`, the minimal Windows Terminal settings live in `windows/terminal/settings.json`, and `JetBrainsMono Nerd Font` is part of the core `winget` baseline. The live prompt uses the standard `oh-my-posh` theme folder under `C:\Program Files (x86)\oh-my-posh\themes`, which matches the current machine and keeps the profile simple.
...
* **Windows prompt assets:** `windows/omp/tokyo.omp.json` captures the current `oh-my-posh` theme, `windows/terminal/settings.json` captures the minimal Terminal defaults, and `JetBrainsMono Nerd Font` is bootstrapped through the core `winget` manifest. The live profile points to the installed theme path so the prompt stays straightforward.
